 ...
 * Inclui APPROVED apenas quando inicio >= hoje - 7 dias (período curto/recente).
 * Exemplos:
 *   Hoje       → inicio = hoje    → inclui APPROVED ✅
 *   Esta semana → inicio = seg    → inclui APPROVED ✅
 *   Este mês   → inicio = 01/mm  → exclui APPROVED ✅ (inicio é antigo)
 *   Este ano   → inicio = 01/01  → exclui APPROVED ✅
 */
function statusFaturamento(inicio: string): string {
  const seteDiasAtras = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
  const dataInicio = new Date(inicio + 'T00:00:00Z')
  return dataInicio >= seteDiasAtras
